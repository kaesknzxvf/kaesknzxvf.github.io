...
10.  메세지 입력 시, 텍스트 필드 크기에 따라 메세지 로그 범위도 변경  
    ...
    //   ChatPerson(this.name, this.imageUrl);
    // }
    ...
    //print('cpimageUrl : ${cpimageUrl}, cpname : ${cpname}');
    ...
    void _scrollUpdater() {
        ...
        WidgetsBinding.instance.addPostFrameCallback((_) {
            //입력도중에 바텀 크기가 바뀌어도 보던 화면 유지해주고 싶음 (미구현)
    ...
    Widget build(BuildContext context) {
        ...
        body: Column(
            ...
            Flexible(
                ...
                //margin: EdgeInsets.only(bottom: _currentbottomheight),
                    ...
                    children: <Widget>[
                        ...
                        child: TextField(
                            ...
                            if (_currentbottomheight != _minbottomheight) {
                                //10. 추가, 텍스트 필드 높이가 변하면 스크롤도 이동해줌 (미구현)
                                _scrollUpdater();
                            }
                            },
                            keyboardType:
